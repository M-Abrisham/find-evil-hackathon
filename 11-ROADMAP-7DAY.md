...
## - [ ] → From Slack --> pull the **starter evidence datasets + example submission + practice MCP endpoint** (Rules L45) — this is the judge-runnable evidence path the Day-7 Try-It-Out needs ("run locally *against provided evidence*"), and the example submission is the only sponsor-provided calibration of judge expectations *(small, ~1h)*
- [ ] ⚙️ **Devpost admin** add Josh To Contibuter
