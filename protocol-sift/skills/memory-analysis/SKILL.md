...
### Evidentiary discipline (anti-hallucination / abstention)

- **Empty output is NOT a clean system.** A plugin that returns zero rows, errors, or no hits means INSUFFICIENT_EVIDENCE for that question, not "nothing malicious." Treat an empty/zero-result as a possible plugin/config/symbol error and re-run or cross-check before concluding.
- **Process anomaly indicators are LEADS, not labels.** A `malfind` hit, a hollowing signature, an orphan parent, or an unusual path is an investigative lead — it does not by itself establish MALICE.
- **Identity before labeling.** Resolve a process/DLL/service identity (full image path, signature, hash, known-good baseline) BEFORE applying any malice label.
- **Dual-use is presumed legitimate.** Built-in and admin/IR tooling (e.g. powershell.exe, wmic.exe, rundll32.exe, procdump, PsExec) is PRESUMED LEGITIMATE until anomalous context (unexpected path, parent, account, timing, arguments, or corroborating malice) is positively evidenced.
- **Sweep before pivot.** Run the broad sweep plugins (psscan/pslist, netscan, malfind, svcscan) across the whole image FIRST to map the field; only then pivot deep on a specific PID. Note any plugin you did NOT run as a Coverage Gap.
- **Quote before you claim.** Every literal you assert (PID, path, hash, IP, command line, account) must appear verbatim in cited plugin output before it enters the report. If you cannot quote it, you cannot claim it — and never normalize or reformat a value that is not in the output exactly as written.
