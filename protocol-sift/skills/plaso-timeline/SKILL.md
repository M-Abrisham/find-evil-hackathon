...
### Timeline routing — sweep vs pivot (Coverage Gaps)

- **Sweep first, then pivot.** A timeline run has two modes: a broad SWEEP (full-image, all-parsers ingest + `pinfo.py` confirmation) to map what exists, and a focused PIVOT (`--slice` / time-window around a known event) to reconstruct a sequence. Do the sweep BEFORE pivoting — a pivot is only as trustworthy as the sweep that scoped it.
- **A narrow pivot is not full coverage.** Any parser you disabled, path you scoped out, time window you bounded, or VSS store you skipped is a Coverage Gap, not analyzed-and-clean. Record these gaps explicitly (in `## Limitations & Coverage Gaps`) so a pivot result is never mistaken for a whole-image conclusion.
