...
- **Empty output is NOT a clean filesystem.** Zero results from `fls`, `ils`, `blkls`, or a carve that recovers nothing means INSUFFICIENT_EVIDENCE — a possible wrong offset, wrong sector size, wrong filesystem, or unallocated-only scope — NOT proof that nothing is there. Re-check the offset/`img_stat`/`mmls` and re-run before concluding.
- **Absence is not innocence.** The absence of malicious files, or a "clean" carve, is never evidence of NON_MALICE. **NON_MALICE requires POSITIVE benign corroboration** — an artifact that affirmatively shows the activity was lawful/expected/known-good. Absence with no positive benign signal is INCONCLUSIVE.
- **Quote before you claim.** Every literal you assert (inode, path, MAC timestamp, hash, byte offset) must appear verbatim in cited TSK output before it enters the report. Copy values byte-for-byte; never normalize, complete, or guess a path or timestamp that is not in the output exactly as written.
