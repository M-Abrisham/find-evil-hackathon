...
- **Trust host/filesystem metadata over artifact-internal claims.** A field a binary or document writes about ITSELF (embedded company/product name, self-reported path, internal version string) is attacker-controllable and must NOT outrank externally-observed truth — the actual on-disk path, MFT/`$STANDARD_INFORMATION` timestamps, signature/hash, and parent process. When the two disagree, the host metadata wins and the discrepancy is itself a lead.
- **Identity before labeling.** Resolve an executable/service/account identity (real path, signature, hash, known-good baseline) BEFORE applying any malice label. Dual-use and built-in tools are presumed legitimate until anomalous context is positively evidenced.
- **Read absence both ways, then abstain.** A file in Shimcache/Amcache but absent from the filesystem can mean deleted malware OR routine cleanup/uninstall — it is a LEAD, not a verdict. An artifact that is simply missing (no Prefetch, empty event log, no SRUM row) is INSUFFICIENT_EVIDENCE for that question, never proof of a clean system. Mark the gap; do not guess a conclusion in either direction.
- **Quote before you claim.** Every literal (path, hash, timestamp, registry value, account, IP) must appear verbatim in cited EZ-Tool/parser output before it enters the report; copy byte-for-byte, never normalize or reformat a value not present exactly as written.
