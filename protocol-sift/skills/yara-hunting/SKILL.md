...
- **A zero-hit scan is NOT a clean target.** No YARA matches means INSUFFICIENT_EVIDENCE for the rules you ran — not "no malware." A rule set only finds what it encodes; absence of a hit is never proof of innocence.
- **A timed-out, errored, or skipped scan = UNSCANNED, not clean.** If `yara` hits a timeout (`-a`), a permission/IO error, an unreadable/locked file, or a path you excluded, that target was not actually covered. Record it as a Coverage Gap and re-run; do not report it as negative.
- **A YARA hit is a LEAD, not a label.** A match (especially on packer/generic/dual-use signatures) is an investigative pointer that demands identity resolution and corroboration before any MALICE call — it does not by itself establish maliciousness.
- **Quote before you claim.** Cite the exact rule name and matched offset/string from the scan output; never normalize or invent a match detail not present in the output verbatim.
